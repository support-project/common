 ...
 * 握りつぶされてしまい本来の例外処理までExceptionが届かず、システムに不具合が起きたときの原因追求が
 * 困難だったというのはよくある例です。
 * 
 * 特にEclipseのようなIDE（統合開発環境）を利用すればExceptionのtry-catch文がないことを警告し、try-catch文の
 * テンプレートなどを自動生成してくれますが、自動生成されたtry-catch文をそのままにしておけばExceptionは
 * そこで握りつぶされてしまいます。
 * 
