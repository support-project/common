	...
	private static void eq(Object expected, Object actual, String parent, boolean db, List<String> ignores) {
		...
		if (Collection.class.isAssignableFrom(expected.getClass())) {
			...
			if (collection1.size() != collection2.size()) {
				throw new AssertionError("[" + parent + "] actual size is " + collection2.size() + ". but expected size is " + collection1.size() + ".");
			}
...
//			//プロパティを持たないオブジェクト
//			if(!expected.equals(actual)) {
//				throw new AssertionError("actual is " + PropertyUtil.reflectionToString(actual) + ". but expected is " + PropertyUtil.reflectionToString(expected) + ".");
//			}
//		}
