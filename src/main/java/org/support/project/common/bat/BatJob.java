...
public class BatJob implements Job {
	...
	public JobResult execute() throws IOException, InterruptedException {
		...
		/*
		 * 参考：http://www.ne.jp/asahi/hishidama/home/tech/java/process.html#Process
		 * 標準出力／エラー出力を別々に取得しようとすると、プログラムの処理がブロッキングに入り、
		 * 終了しなくなることがある。
		 * 標準出力／エラー出力を別々のスレッドにすれば取得できるが、Webアプリなどでは、
		 * 新たにスレッドを生成することができない(エラーになる)。
		 * このため、標準出力／エラー出力は同じストリームで取得する。
		 */
