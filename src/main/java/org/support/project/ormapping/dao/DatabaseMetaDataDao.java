...
public class DatabaseMetaDataDao extends AbstractDao {
	/** ログ */
	private static Log LOG = LogFactory.getLog(DatabaseMetaDataDao.class);
	...
	private List<TableDefinition> readTableDefinition(Connection con) throws SQLException, SystemException, NoSuchMethodException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (!"INDEX".equals(definition.getTable_type())
						&& !"SEQUENCE".equals(definition.getTable_type())) {
	...
	private List<ColumnDefinition> readColumnDefinition(Connection con, TableDefinition tableDefinition) throws SQLException, SystemException, NoSuchMethodException {
		...
		try {
			...
			if (ORMappingParameter.DRIVER_NAME_H2.equals(driverClass)) {
				//H2SQL
				rs = databaseMetaData.getPrimaryKeys(null,ConnectionManager.getInstance().getSchema().toUpperCase(), tableDefinition.getTable_name());
			} else {
				//Postgresql
