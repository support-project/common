...
public class EntityMaker {
	...
	/** ログ */
	private static Log LOG = LogFactory.getLog(EntityMaker.class);
	


	/**
	 * auto_gen_for_or_mapping.properties の設定ファイルに従い、
	 * Entityクラスを作成する
	 * 
	 * @param args パラメータ(いったん今は何も使わない。後で、設定ファイル名を渡すとか考える)
	 * @throws Exception 
	...
	/**
	 * エンティティクラスを生成する
	 * エンティティは、後で手で修正したい事もあるので、以下の構成にする
	 * 
	 * xxx.entity.gen
	 *     GenXxxxEntity
	 *     テーブル定義から自動生成したカラム設定が書かれる(このクラスは手で修正はしない)
	 *     アブストラクトにする
	 * 
	 * xxx.entityパッケージ
	 *     XxxxEntity
	 *     上記GenEntityを継承したクラス。いったん箱だけ用意する
	 * 
	 */
	private void make() {
