...
public class DefaultTableInsertMethodCreator {
	...
	private static Log log = LogFactory
			.getLog(DefaultTableSelectMethodCreator.class);
	...
	private void writeInsertOnUser(PrintWriter pw) {
		...
		if (StringUtils.isNotEmpty(config.getCommonInsertUserName())) {
			// ユーザ名の列定義
			List<ColumnDefinition> columnDefinitions = config
			...
			if (updateUserColumn != null) {
				// 登録ユーザをセット
				String feildName = nameConvertor
						.colmnNameToFeildName(updateUserColumn.getColumn_name());
				pw.print("\t\tentity.");
				pw.print(helper.feildNameToSetter(feildName));
				pw.println("(user);");
			}
			if (updateDatetimeColumn != null) {
				// 登録ユーザをセット
				String feildName = nameConvertor
						.colmnNameToFeildName(updateDatetimeColumn.getColumn_name());
				pw.print("\t\tentity.");
				pw.print(helper.feildNameToSetter(feildName));
				pw.println("(new Timestamp(new java.util.Date().getTime()));");
			}
	...
	private void writePhysicalInsert(PrintWriter pw) {
		...
		if (keygen) {
			pw.println("Class<?> type = PropertyUtil.getPropertyType(entity, \"" 
					+ nameConvertor.colmnNameToFeildName(keycol.getColumn_name()) + "\");");
